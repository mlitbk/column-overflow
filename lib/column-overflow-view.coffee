...
module.exports =
class ColumnOverflowView extends View

  @content: ->
    @div class: 'column-overflow-wrapper'
  ...
  initialize: (@editorView) ->
    ...
    @subscribe atom.config.observe 'column-overflow.column', callNow: false, =>
      @subscribeToBuffer()
  ...
  findOverflows: ->
    column = atom.config.get('column-overflow.column')
    text = @buffer.getText()
    overflows = []
    row = 0
